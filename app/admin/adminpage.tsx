...
function StudentsDetailTab({ mS, mT, allStudents }: { mS: Student[]; mT: Teacher[]; allStudents: Student[]; }) {
  ...
  // ✅ FIX: تحميل التقييمات من Supabase بدل localStorage
  React.useEffect(() => {
    if (!mS.length) return;
    const ids = mS.map(s => s.id);
    supabase.from('sem_evals').select('*').in('student_id', ids).then(({ data }) => {
      if (data) {
  ...
  const [hifzData, setHifzData] = React.useState<Record<string,any>>({});  const filtered = (!semF || !teachF) ? [] : mS.filter(s => s.teacherId === parseInt(teachF));
...
function SchedulesTab() {
  ...
  // ✅ FIX: تحميل الجداول من Supabase مباشرة بدل localStorage
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        ...
        if (data) {
          ...
          const rows: any[] = data.flatMap((record: any) => {
            const tch = allT.find(t => t.id === record.teacher_id);
            return (record.rows || []).map((r: any, i: number) => ({
              id: record.teacher_id * 1000 + i,
              teacherId: record.teacher_id,
              teacherName: tch?.name || '',
              mosqueId: record.mosque_id || tch?.mosqueId || 0,
        ...
        console.error('SchedulesTab load error:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // ✅ FIX: نبين كل الجداول أو نفلتر بالمسجد
  const filtered = matchedMosque
    ? scheduleData.filter(s => s.mosqueId===matchedMosque.id || s.mosqueName===matchedMosque.name)
    : scheduleData;
  const maxRows = Math.max(4, ...DAYS.map(d=>filtered.filter(s=>s.day===d).length));
  if (loading) return <div style={{textAlign:'center',padding:60,color:'var(--txt3)'}}>⏳ جاري تحميل الجداول...</div>;
  return (
    <div className="main pgin">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12,marginBottom:16}}>
